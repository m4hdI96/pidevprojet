...
exports.createTournament = async (req, res, next) => {
    try{
        ...
        console.log(newTournament);
         tournament.create(req.body).then(data =>{
            console.log(data);
         }).catch(err =>{
            console.log(err);
         });
...
exports.updateTournament = (req, res, next) => {
    try {
        ...
        tournament.findOneAndUpdate({ _id: req.params.id }, req.body).then(result => {
            res.status(200).json({
...
exports.getTournamentById = async (req, res, next) => {
    try {
        ...
        const tourement = await tournament.findById(req.params.id)
            return res.status(200).json({
                message: 'Tournament fetched successfully!',
                tournament: tourement
            });
